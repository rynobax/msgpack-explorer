  ...
  render: _self => {
    ...
    let rec parseHex =
      ...
      | [a, ...rest] =>
        switch (a) {
        /* NIL */
        | 0xc0 => (<Cell header="Nil" raw=[a] color=Color.nil key />, rest)

        /* BOOLEANS */
        | 0xc2 => (
            <Cell header="false" raw=[a] color=Color.boolean key />,
            rest,
          )

        | 0xc3 => (
            <Cell header="true" raw=[a] color=Color.boolean key />,
            rest,
          )
    ...
    let cells = parseHexUntilDone(capitalHexList)->List.rev->Array.of_list;
